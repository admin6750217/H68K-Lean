...
#修改默认连接数
CONNTRACK_SYSCTL="./package/kernel/linux/files/sysctl-nf-conntrack.conf"
if [ -f "$CONNTRACK_SYSCTL" ]; then
	sed -i 's/net\.netfilter\.nf_conntrack_max=.*/net.netfilter.nf_conntrack_max=655200/' "$CONNTRACK_SYSCTL"
fi
...
if [[ "${WRT_CONFIG^^}" == "ROCKCHIP" ]]; then
	...
	if [ -f "$ROCKCHIP_IMAGE" ] && grep -q "define Device/hinlink_opc-h68k" "$ROCKCHIP_IMAGE"; then
		perl -0pi -e 's/(define Device\/hinlink_opc-h68k\n\$\(call Device\/hinlink_common\)\n  DEVICE_MODEL := OPC-H68K\n  SOC := rk3568\n)/$1  DEVICE_PACKAGES += -kmod-r8125-rss -kmod-drm-rockchip -kmod-drm-panfrost -kmod-rockchip-hdmirx -kmod-video-core -kmod-video-uvc\n/' "$ROCKCHIP_IMAGE"
	fi
